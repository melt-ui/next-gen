...
<Preview>
	<Slider value={30} {...controls}>
		{#snippet children(slider)}
			<div
				class="group relative mx-auto p-3 outline-none {slider.orientation === 'horizontal'
					? 'w-[350px]'
					: 'h-[350px] w-[50px]'}"
				{...slider.root}
			>
				<div
					class="absolute rounded-full bg-gray-500 {slider.orientation === 'horizontal'
				...
				<div
					class="group-focus-visible:ring-accent-300 absolute size-6 rounded-md bg-white outline-none
					transition-all group-focus-visible:ring group-focus-visible:ring-offset-2
					group-focus-visible:ring-offset-black data-[dragging]:transition-none
